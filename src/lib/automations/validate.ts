  ...
  switch (step.step_type) {
    ...
    case 'send_webhook':
      ...
      try {
        ...
        // Save-time SSRF guard: reject blocked hostname literals
        // (localhost, etc.) and IP-literal hosts in private CIDRs.
        // Named hostnames pass here; engine.ts performs the DNS-resolved
        // private-IP check at exec time (assertPublicHttpUrl) because
        // validate is sync and cannot await dns.lookup.
        ...
        if (blockedLiterals.has(host) || (isIpLiteral && isPrivateIp(host))) {
          issues.push({
            path: `${path}.url`,
            message: 'webhook URL points at a blocked (private) host',
          });
        }
      } catch {
        issues.push({
