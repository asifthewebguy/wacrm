...
async function processMessage(
  ...
  // Webhook idempotency: Meta replays webhook POSTs (retries, signed-ack
  // races). The partial UNIQUE INDEX on messages.message_id (migration
  // 011) lets us upsert + ignoreDuplicates so a replay is silently
  // skipped at the DB layer. We then check whether the row actually
  // inserted before running side effects (unread_count++, automations).
  ...
  // Stop here — side effects must not double-fire on a Meta replay.
  if (!inserted || inserted.length === 0) {
    console.log(
      '[webhook] duplicate message replay, skipping side effects:',
      message.id
    );
    return;
  }
